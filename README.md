...
本リポジトリでは、git push をトリガーとして GitHub Actions による
簡易的な CI を実行しています。

### 現在実施しているチェック
- 必須ファイル（index.html, script.js など）の存在チェック
- JavaScript（script.js）の構文チェック

これにより、明らかな構文エラーやファイル欠落を
早い段階で検出し、リポジトリの最低限の品質を担保しています。
...
- pull request 時に実行する CI（チェック強化、役割分離）
- 構文チェックに加えた簡易テストの追
